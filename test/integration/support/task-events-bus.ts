...
export class TestTaskEventsBus {
  private readonly eventsByTaskId = new mutable.HashMap<
    number,
    mutable.ArrayBuffer<TestTaskEvent>
  >();
  private readonly startedWaiters = new mutable.HashMap<
    number,
    mutable.ArrayBuffer<CountedDeferred<StartedTaskEvent>>
  >();
  private readonly completedWaiters = new mutable.HashMap<
    number,
    mutable.ArrayBuffer<CountedDeferred<CompletedTaskEvent>>
  >();
  private readonly failedWaiters = new mutable.HashMap<
    number,
    mutable.ArrayBuffer<CountedDeferred<FailedTaskEvent>>
  >();
  ...
  waitForStarted(
    ...
    return this.findEvent(taskId, TestTaskEventType.started, occurrence).match({
      ...
      none: () =>
        this.registerWaiter(taskId, occurrence, this.startedWaiters).promise,
    });
  }
  ...
  waitForCompleted(
    taskId: number,
    occurrence: number = 1,
  ): Promise<CompletedTaskEvent> {
    return this.findEvent(
      taskId,
      TestTaskEventType.completed,
      occurrence,
    ).match({
      some: (event) => Promise.resolve(event as CompletedTaskEvent),
      none: () =>
        this.registerWaiter(taskId, occurrence, this.completedWaiters).promise,
    });
  }
  ...
  waitForFailed(
    ...
    return this.findEvent(taskId, TestTaskEventType.failed, occurrence).match({
      ...
      none: () =>
        this.registerWaiter(taskId, occurrence, this.failedWaiters).promise,
    });
  }
  ...
  private findEvent(
    ...
    return this.events(taskId)
      ...
      .drop(occurrence - 1).headOption;
  }

  private registerWaiter<TEvent extends TestTaskEvent>(
    taskId: number,
    occurrence: number,
    waitersMap: mutable.HashMap<
      number,
      mutable.ArrayBuffer<CountedDeferred<TEvent>>
    >,
  ): Deferred<TEvent> {
  ...
  private resolveWaiters<TEvent extends TestTaskEvent>(
    ...
    waitersMap.get(taskId).foreach((waiters) => {
      const matched = this.events(taskId).count(
        (storedEvent) => storedEvent.type === event.type,
      );
